#!/usr/bin/env bash
# Formal V7: sequential Task0..Task5, with only each S3 training stage using
# one 3-rank DDP job on GPU0, GPU1 and GPU2.
set -euo pipefail
...
RUN_ROOT="${RUN_ROOT:-/data/ckpt/zhaozhuofan/Hyper-LLaVA-runs/v7_ucit_formal_3gpu_seed42}"
TRAINING_GPUS="${TRAINING_GPUS:-0,1,2}"
...
if requested != [0, 1, 2]:
    raise SystemExit("formal launcher is pinned to GPU0,1,2")
...
if busy:
    raise SystemExit("GPU0-2 availability gate failed: {}".format(busy))
print("GPU0-2 availability gate passed: {}".format({i: state[i] for i in requested}))
PY
...
for row in "${rows[@]:1}"; do
  ...
  command=(
    ...
    --device cuda:0 --training-world-size 3 --training-gpus "${TRAINING_GPUS}"
    --training-gradient-accumulation-steps 21 --distributed-backend nccl
    --skip-eval
  )
