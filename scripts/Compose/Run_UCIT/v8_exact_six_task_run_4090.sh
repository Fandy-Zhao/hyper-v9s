#!/usr/bin/env bash
# Formal V8-Exact, GPU-count-adaptive: sequential Task0..Task5 with one
...
RUN_ROOT="${RUN_ROOT:-/data/ckpt/xukunlun/CIT/zzf_runs/v8_exact_full6_20260912}"
V8_CONFIG="${V8_CONFIG:-${REPO}/configs/v8_exact_accelerated.yaml}"
V8_QUERY_CACHE_ROOT="${V8_QUERY_CACHE_ROOT:-/data/ckpt/xukunlun/CIT/zzf_runs/v8_query_cache_rebound_20260912}"
QUERY_CACHE_MANIFEST="${QUERY_CACHE_MANIFEST:-${V8_QUERY_CACHE_ROOT}/query_cache_manifest.json}"
# Original UCIT test evaluation has no migrated test-query cache; it uses three
# idle physical GPUs after each task commit while the next task has not started.
EVAL_GPUS="${EVAL_GPUS:-1,3,4,5}"
...
# The formal 4090 recipe is explicitly pinned to the verified 4-card pool.
# Positional / $GPUS overrides remain available for a deliberate operator change.
GPU_ARG="${1:-${GPUS:-1,3,4,5}}"                 # positional > $GPUS > verified 4-GPU default
...
for required in "${method_config}" "${model_path}" "${vision_tower}" "${projector_path}" "${image_folder}" "${V8_CONFIG}" "${V8_QUERY_CACHE_ROOT}" "${QUERY_CACHE_MANIFEST}"; do
...
for row in "${rows[@]:1}"; do
  ...
  command=(
    ...
    --query-cache-manifest "${QUERY_CACHE_MANIFEST}"
    --compose-v8-config "${V8_CONFIG}" --compose-v8-query-cache-root "${V8_QUERY_CACHE_ROOT}"
  ...
  # Immediate lower-triangle evaluation is a gate before the next task starts.
  evaluate_completed_stage "${task_index}"
done

# All 21 cells now exist from the per-task gates; validate and summarize once.
"${PY}" -m compose.eval.v7_formal_ucit_eval \
  --root "${RUN_ROOT}" --formal-config "${FORMAL_CONFIG}" \
  --method-config "${METHOD_CONFIG}" --python "${PY}" --gpus "${EVAL_GPUS}"
