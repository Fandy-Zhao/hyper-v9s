...
for ((index=0; index<chunks; index++)); do
  CUDA_VISIBLE_DEVICES="${gpus[$index]}" python -m compose.eval.eval_task \
    ...
    --expert-ids "$EXPERT_IDS" --gates "$GATES" --normalization "$NORMALIZATION" \
    --num-chunks "$chunks" --chunk-idx "$index" --device cuda:0 &
  pids+=("$!")
done
for pid in "${pids[@]}"; do wait "$pid"; done
: > "$RESULT_DIR/predictions.jsonl"
for ((index=0; index<chunks; index++)); do cat "$RESULT_DIR/${chunks}_${index}.jsonl" >> "$RESULT_DIR/predictions.jsonl"; done
python -m compose.eval.metrics --annotation-file "$QUESTION_FILE" \
  --predictions-file "$RESULT_DIR/predictions.jsonl" --output-file "$RESULT_DIR/metrics.json"
