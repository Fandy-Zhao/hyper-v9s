...
# 依据：configs/v6_ucit_formal_locked.yaml（formal locked config v4）
# 使用方式：
#   bash scripts/v6_ucit/six_task_run.sh <seed> [gpus]
#   seed 默认 42；gpus 默认 4,5,6,7
#
# v4 训练策略（2026-08-06）：单卡 batch-1 x accum 24 = global 24。
#   - 4 卡 DDP 下 expandable_segments 报 not supported；eager attention
#     单样本 forward 峰值 ~22.75GB 贴 24GB 上限，DDP 每 rank 额外开销 OOM。
#   - dry-run 已验证单卡 batch-1 + expandable_segments（README 明确支持
#     "or keep batch-1 single-card"）；实测 3.25 s/step（global 24）。
#   - TRAIN_GPU 指定训练卡（默认 4，空闲；GPU 5-7 保留给其他用户/任务）。
set -Eeuo pipefail
...
echo "== seed $SEED 六任务正式运行 (train GPU $TRAIN_GPU, eval GPUs $GPUS) =="
mkdir -p "$RUN_ROOT"
...
run_stage task0_done env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --gpus "$TRAIN_GPU" --master-port $((MASTER_PORT + 0)) \
  --config "$CONFIG"
...
run_stage task1_done env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --gpus "$TRAIN_GPU" --master-port $((MASTER_PORT + 1)) \
  --config "$CONFIG"
...
run_stage task2_done env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --task-id 2 --gpus "$TRAIN_GPU" --master-port $((MASTER_PORT + 2)) \
  --config "$CONFIG"
...
run_stage task3_done env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --task-id 3 --gpus "$TRAIN_GPU" --master-port $((MASTER_PORT + 3)) \
  --config "$CONFIG"
...
run_stage task4_done env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --task-id 4 --gpus "$TRAIN_GPU" --master-port $((MASTER_PORT + 4)) \
  --config "$CONFIG"
...
run_stage task5_done env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --task-id 5 --gpus "$TRAIN_GPU" --master-port $((MASTER_PORT + 5)) \
  --config "$CONFIG"
