"""V8 teacher runner: the few-shot Answer-Supervised Expert Teacher.

Invoked by :mod:`compose.v8.workflow` as stage S2/S3 of the formal V8 pipeline;
it is not a user-facing entry point.  The formal per-task entry is
``python -m compose.experiments.v8_task_run``.

It runs on a **train** subset or on the validation split, never on test, and
reads the committed pool of the *previous* task (``--checkpoint-dir``), so the
capability question it answers is the one the next task actually faces.
