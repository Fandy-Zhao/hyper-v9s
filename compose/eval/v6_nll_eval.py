"""V6 answer-NLL calculator over arbitrary per-sample expert sets (E12).

Loads a compose checkpoint (old experts + optionally candidates), then
for each sample and each requested expert set computes the teacher-forced
answer NLL using the Stage-04 scorer (answer-masked, token mean). Output
is ``{sample_id: {set_key: mean_nll}}``.

This is the workhorse behind the answer-supervised teacher search, the
candidate conditional-gain validation and the residual judgment on real
UCIT data.
"""
...
def main() -> None:
    ...
    for record in records:
        ...
        for set_key, expert_ids in sorted(selections[sample_id].items()):
            ...
            padded_ids, padded_gates = _pad(ids)
...
def _pad(expert_ids: Sequence[int]) -> Tuple[Tuple[int, ...], Tuple[float, ...]]:
    ids = tuple(int(value) for value in expert_ids)
    gates = tuple(1.0 for _ in ids)
    while len(ids) < 2:
        ids = ids + (PAD_EXPERT_ID,)
        gates = gates + (0.0,)
    return ids, gates
