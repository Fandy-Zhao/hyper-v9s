"""V6 expert assembly: candidate state dict -> standard compose checkpoint
(E12 commit-time artifact writer).

Loads the base model, injects compose adapters, registers the old experts
(optional) and the new expert id, copies the candidate LoRA weights from a
``candidate_<id>.pt`` payload, then writes a standard compose expert
checkpoint (compose_experts.json + compose_experts.bin) whose directory can
be consumed by ``load_compose_model`` and the original eval entry.
"""
...
def main() -> None:
    ...
    parser.add_argument("--candidate-state-dict", required=True)
    ...
    pool.register(args.expert_id, name="expert_{:04d}".format(args.expert_id))

    payload = torch.load(args.candidate_state_dict, map_location="cpu", weights_only=False)
    if int(payload["candidate_id"]) != args.expert_id:
        raise ValueError(
            "candidate id {} != expert id {}".format(
                payload["candidate_id"], args.expert_id
            )
        )
    with torch.no_grad():
