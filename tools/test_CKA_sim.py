"""作用：计算和可视化模型层间 CKA 相似度，用于分析不同任务训练后层表示变化。"""
...
class CustomDataset(Dataset):
    """作用：CustomDataset 类封装当前模块中的相关状态和操作。"""
...
def centering(K):
    """作用：执行 centering 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
    n = K.shape[0]
    unit = np.ones([n, n])
    I = np.eye(n)
    H = I - unit / n
...
def rbf(X, sigma=None):
    """作用：执行 rbf 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
    GX = np.dot(X, X.T)
...
def kernel_HSIC(X, Y, sigma):
    """作用：执行 kernel_HSIC 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
    return np.sum(centering(rbf(X, sigma)) * centering(rbf(Y, sigma)))


def linear_HSIC(X, Y):
    """作用：执行 linear_HSIC 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
    L_X = np.dot(X, X.T)
    L_Y = np.dot(Y, Y.T)
    return np.sum(centering(L_X) * centering(L_Y))

def linear_CKA(X, Y):
    """作用：执行 linear_CKA 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
...
def kernel_CKA(X, Y, sigma=None):
    """作用：执行 kernel_CKA 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
...
def hook_fn(name, activation_values):
    """作用：执行 hook_fn 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
    def hook(module, input, output):
        """作用：执行 hook 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
        if output.size(1) >= 2:
...
def register_hooks(model, activation_values):
    """作用：执行 register_hooks 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
...
def create_data_loader(questions, image_folder, tokenizer, image_processor, model_config, batch_size=1, num_workers=4):
    """作用：执行 create_data_loader 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
