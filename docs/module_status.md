...
| Compose V7 Fixed-Query Cache | `compose/v7/query_cache.py`, `compose/eval/precompute_v7_queries.py` | Standalone full precompute of V7 fixed queries + task centers (GPU0+GPU1) | Validated (cache READY, not yet wired into S3) | 28 new CPU tests + single-vs-dual gate (bit-exact) + 48-test regression | 230,780 queries cached; atomic/resumable; reuse change point `compose/train/data.py:369` (report §29); main flow untouched |
