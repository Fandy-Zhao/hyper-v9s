...
| Compose V7 Fixed-Query Cache | `compose/v7/query_cache.py`, `compose/v7/cache_to_s1.py`, `compose/v7/cached_selections.py`, `compose/eval/precompute_v7_queries.py`, `compose/eval/v7_cache_live_gate.py`, `compose/eval/v7_cache_precheck.py` | Full precompute + downstream reuse of V7 fixed queries / task centers | Code complete (adaptation); Phase A independent re-audit PASS at HEAD ea816a3 (18/18 splits + 6/6 centers bit-exact recomputed, 230,780 rows); QUERY_NUMERICAL_EQUIVALENCE + TOP2_ROUTING_EQUIVALENCE PASS on GPU1 (bit-exact) and re-PASS on GPU0 at final HEAD | 28 cache-audit + 7 cache-to-s1 + 7 cached-selections CPU tests; `tests/compose` 500/502 (2 = pre-existing `java`-less env limits) | S1/S3/S4/S5/S6 + 21-cell eval consume cache rows (`encoder_calls=0`), sample_id sequence fail-closed, content binding; method unchanged; cache-vs-live gates bit-exact on 2 tasks x 128 samples (cosine_min 0.99999976 >= 1-1e-6, route agreement 1.0) |
