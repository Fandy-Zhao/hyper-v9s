...
| Compose V7 Fixed-Query Cache | `compose/v7/query_cache.py`, `compose/v7/cache_to_s1.py`, `compose/v7/cached_selections.py`, `compose/eval/precompute_v7_queries.py` | Full precompute + downstream reuse of V7 fixed queries / task centers | Code complete (adaptation), GPU gates pending idle GPU0/1 | 28 cache-audit + 7 cache-to-s1 + 7 cached-selections CPU tests; `tests/compose` 500/502 (2 = pre-existing `java`-less env limits) | S1/S3/S4/S5/S6 + 21-cell eval consume cache rows (`encoder_calls=0`), sample_id sequence fail-closed, content binding; method unchanged; formal run blocked on other users' GPU0/1 jobs |
