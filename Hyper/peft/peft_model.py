...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType.MOE_LORA_Hyper: HyperMOELoraModel,
}
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self, adapter_name: str):
        """作用：执行 _setup_prompt_encoder 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _split_kwargs(cls, kwargs: Dict[str, Any]):
        """作用：执行 _split_kwargs 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def base_model_torch_dtype(self):
        """作用：执行 base_model_torch_dtype 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        return getattr(self.base_model, "dtype", None)

    @property
    def active_peft_config(self):
        """作用：执行 active_peft_config 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    **Attributes**:
    ...
    def _prefix_tuning_forward(
        ...
        **kwargs,
        ...
        """作用：执行 _prefix_tuning_forward 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        batch_size = input_ids.shape[0]
...
class PeftModelForCausalLM(PeftModel):
    ...
    def generate(self, **kwargs):
        """作用：执行 generate 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
...
class PeftModelForCausalLMLORAMOE(PeftModelForCausalLM):
    ...
    """作用：PeftModelForCausalLMLORAMOE 类封装模型结构、配置或前向传播相关逻辑。"""
    def __init__(self, model, peft_config: PeftConfig, adapter_name="default"):
