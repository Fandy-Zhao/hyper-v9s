...
class IA3Model(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _prepare_ia3_config(peft_config, model_config):
        """作用：执行 _prepare_ia3_config 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
...
def mark_only_ia3_as_trainable(model: nn.Module) -> None:
    """作用：执行 mark_only_ia3_as_trainable 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
...
class IA3Layer:
    """作用：IA3Layer 类封装一个参数高效微调组件或 LoRA expert 结构。"""
    def __init__(
        self,
        in_features: int,
        out_features: int,
    ...
    def reset_ia3_parameters(self, adapter_name):
        """作用：执行 reset_ia3_parameters 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
