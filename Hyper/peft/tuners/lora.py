# coding=utf-8
"""作用：实现 Hyper/PEFT 的参数高效微调 tuner，包括 LoRA、MoE-LoRA、prompt/prefix tuning 等适配器。"""
...
class LoraConfig(PeftConfig):
    ...
    def __post_init__(self):
        """作用：实现 Python 特殊方法 __post_init__，用于配合对象协议或框架调用。"""
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def add_adapter(self, adapter_name, config=None):
        """作用：执行 add_adapter 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _check_quantization_dependency(self):
        """作用：执行 _check_quantization_dependency 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _check_target_module_exists(self, lora_config, key):
        """作用：执行 _check_target_module_exists 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        """作用：执行 _create_new_module 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _find_and_replace(self, adapter_name):
        """作用：执行 _find_and_replace 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _replace_module(self, parent_module, child_name, new_module, old_module):
        """作用：执行 _replace_module 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _set_adapter_layers(self, enabled=True):
        """作用：执行 _set_adapter_layers 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        for module in self.model.modules():
            if isinstance(module, LoraLayer):
                module.disable_adapters = False if enabled else True

    def enable_adapter_layers(self):
        """作用：执行 enable_adapter_layers 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        self._set_adapter_layers(enabled=True)

    def disable_adapter_layers(self):
        """作用：执行 disable_adapter_layers 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _prepare_lora_config(peft_config, model_config):
        """作用：执行 _prepare_lora_config 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _unload_and_optionally_merge(self, merge=True):
        """作用：执行 _unload_and_optionally_merge 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def _svd_weighted_adapter(self, adapters, weights, new_rank, target, target_lora_A, target_lora_B):
        """作用：执行 _svd_weighted_adapter 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    """作用：执行 mark_only_lora_as_trainable 函数对应的工具逻辑，供当前脚本或其他模块复用。"""
...
class LoraLayer:
    """作用：LoraLayer 类封装一个参数高效微调组件或 LoRA expert 结构。"""
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        """作用：执行 update_layer 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def update_layer_conv2d(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        """作用：执行 update_layer_conv2d 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        if lora_dropout > 0.0:
            lora_dropout_layer = nn.Dropout(p=lora_dropout)
    ...
    def update_layer_embedding(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        """作用：执行 update_layer_embedding 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        if lora_dropout > 0.0:
            lora_dropout_layer = nn.Dropout(p=lora_dropout)
    ...
    def reset_lora_parameters(self, adapter_name):
        """作用：执行 reset_lora_parameters 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
...
class Linear(nn.Linear, LoraLayer):
    ...
    """作用：Linear 类封装一个参数高效微调组件或 LoRA expert 结构。"""
    def __init__(
        self,
        adapter_name: str,
        in_features: int,
    ...
    def merge(self):
        """作用：执行 merge 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
    ...
    def unmerge(self):
        """作用：执行 unmerge 方法对应的模块内部逻辑，通常由训练、推理或服务流程间接调用。"""
        if self.active_adapter not in self.lora_A.keys():
            return
        if not self.merged:
...
class Embedding(nn.Embedding, LoraLayer):
    ...
    """作用：Embedding 类封装一个参数高效微调组件或 LoRA expert 结构。"""
    def __init__(
        self,
        adapter_name: str,
        num_embeddings: int,
...
class Conv2d(nn.Conv2d, LoraLayer):
    ...
    """作用：Conv2d 类封装一个参数高效微调组件或 LoRA expert 结构。"""
    def __init__(
        self,
        adapter_name: str,
        in_channels: int,
...
if is_bnb_available():
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, LoraLayer):
        # Lora implemented in a dense layer
        """作用：Linear8bitLt 类封装一个参数高效微调组件或 LoRA expert 结构。"""
        def __init__(
            self,
            adapter_name,
            in_features,
    ...
    if is_bnb_4bit_available():
        ...
        class Linear4bit(bnb.nn.Linear4bit, LoraLayer):
            # Lora implemented in a dense layer
            """作用：Linear4bit 类封装一个参数高效微调组件或 LoRA expert 结构。"""
            def __init__(
                self,
                adapter_name,
                in_features,
